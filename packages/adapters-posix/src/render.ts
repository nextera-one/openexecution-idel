 ...
 *
 * CANONICAL COPY. This file lives in `@openexecution/adapters-posix` and is the
 * source of truth. `@openexecution/adapters-powershell` keeps a byte-identical
 * copy at `packages/adapters-powershell/src/render.ts` to avoid a cross-package
 * runtime dependency (both packages depend only on `@openexecution/types`).
